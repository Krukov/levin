...
def test_simple_func_profile_overhead():
    profile_result = Profile()
    t1 = time.time()
    samples.simple()
    t2 = time.time()
...
def test_deep_func_profile():
    ...
    assert result[2].code.strip() == "return _in() or s"
    assert result[2].func_line == 6

    assert 0.1 < result[2].time < 0.2


def test_deep_func_profile_2():
    profile_result = Profile(depth=2)
    profile_result.run(samples.deep)
    ...
    assert result[3].code.strip() == "return simple(s)"
...
def test_deep_func_profile_3():
    profile_result = Profile(depth=3)
    profile_result.run(samples.deep)
    ...
    assert result[4].code.strip() == "time.sleep(0.1)"
    assert result[4].depth == 3
...
def test_with_recursion_func_profile():
    profile_result = Profile()
...
def test_with_recursion_func_profile_depth2():
    profile_result = Profile(depth=2)
    profile_result.run(samples.with_recursion)
...
def test_method_profile_exception():
    profile_result = Profile()
...
def test_deep_func_simple_profile_2():
    ...
    assert result[2].code.strip() == "return _in() or s", result
    assert result[2].depth == 1
    assert result[2].func_line == 6
    ...
    assert 0.1 < result[3].time < 0.2
    assert 0.1 < result[2].time < 0.2
    assert len(result) == 4, profile_result._target_func
